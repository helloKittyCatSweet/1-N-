...
export default {
  ...
  computed: {
    // 过滤：按精选标签
    filtered() {
      const tag = this.activeFeatured;

      return (this.lists || []).filter((it) => {
        const itemTag = (it.tag || it.category || it.topic || '').toString().toLowerCase();
        return tag ? (itemTag ? itemTag.indexOf(tag) > -1 : true) : true;
      });
    }
  },
  onLoad() {
